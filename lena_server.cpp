		...
		if (s[0]=='-') {
			...
			if (s[1]=='-') {
				for (ret=0;ret<LENGTH;ret++)
					if (!strcmp(s+2,OPTS[ret])) break;
			} else {
				if (strlen(s)==2) {
					for (ret=0;ret<LENGTH;ret++)
						if (s[1]==OPTC[ret]) break;
				} else ret=LENGTH;
			}
			if (ret==LENGTH) goto err;
			switch (ret) {
				case 0:CLIENT_NAME=args[++i];break;
				case 1:CLIENT_OS=args[++i];break;
				case 2:CLIENT_DISPLAY_WORKDIR=args[++i];break;
				case 3:CLIENT_REAL_WORKDIR=args[++i];break;
			}
		} else goto err;
		continue;
		err:;
